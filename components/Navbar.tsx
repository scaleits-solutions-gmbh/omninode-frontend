...
export function Navbar() {
  return (
    <header className="sticky top-0 z-40 w-full border-b border-white/5 bg-background/70 backdrop-blur">
      <a href="#overview" className="sr-only focus:not-sr-only focus:absolute left-2 top-2 bg-white text-black px-3 py-2 rounded">Skip to content</a>
      <div className="container-xl flex h-16 items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm text-white/80">
          <Link href="#overview" className="hover:text-white transition-colors">Product</Link>
          <Link href="#vision" className="hover:text-white transition-colors">Solutions</Link>
          <Link href="#applications" className="hover:text-white transition-colors">About Us</Link>
        </nav>
      ...
      <div className="md:hidden peer-checked:block hidden border-t border-white/5 bg-background/95">
        <div className="container-xl py-3 flex flex-col gap-3 text-white/90">
          ...
          <Link href="#vision">Solutions</Link>
          <Link href="#applications">About Us</Link>
        </div>
      </div>
    </header>
  );
