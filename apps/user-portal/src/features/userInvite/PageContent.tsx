...
import UserInviteStep1 from "./components/UserInviteStep1";
import UserInviteStep2 from "./components/UserInviteStep2";
import { activateUserInvite, getUserInvite } from "@/lib/apiClient/userInvite";
...
import UserInviteStep3 from "./components/UserInviteStep3";
import { ActivatingUser } from "./types/ActivatingUser";
import LoadingUserInviteCard from "./components/UserInviteCardSkeleton";
...
export default function UserInvitePageContent() {
  ...
  const activateInviteMutation = useMutation({
    mutationFn: ({ companyUserInviteId, userData }: { 
    ...
    onSuccess: () => {
      toast.success("Invite activated");
      router.push("/login");
    },
    onError: (error) => {
  ...
  const handleStep3Activate = async (step3Data: Partial<ActivatingUser>) => {
    ...
    if (data.id && finalUserData.firstName && finalUserData.lastName && finalUserData.position && finalUserData.password) {
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-6 items-center">
        ...
        {step === 2 && (
          <UserInviteStep2 password={userData.password} onNext={handleStep2Next} onBack={handleStep2Back} />
        )}
        {step === 3 && (
          <UserInviteStep3
