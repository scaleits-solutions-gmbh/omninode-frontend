...
import { AdminConsoleLoginService, AdminConsoleLoginJwtPayload, ResultType } from "@scaleits-solutions-gmbh/services";
import { decodeJwt } from "jose";
import { handleServiceError } from "@/lib/utils/misc/api-error-handler";


export async function POST(request: Request): Promise<NextResponse<null|unknown>> {
    try {
        ...
        if (result.resultType === ResultType.SUCCESS && result.result) {
            ...
            const decodedToken = decodeJwt<AdminConsoleLoginJwtPayload>(result.result.token);
