...
import { getManagementConsoleAccessOptions, ManagementConsoleAccess } from "@scaleits-solutions-gmbh/services";
import { useMutation } from "@tanstack/react-query";
import { toast } from "sonner";
import { useState } from "react";
import { sendCompanyUserInvite } from "@/lib/apiClient/company/company";
...
export default function InviteUserPopup() {
  ...
  const isRoleValid = (value: string) => {
    ...
    if (!Object.values(ManagementConsoleAccess).includes(value as ManagementConsoleAccess)) {
      return "Invalid role";
    }
    return undefined;
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="default" size="sm">Invite User</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        ...
        >
          ...
          >
            {(field) => (
              <div className="space-y-2">
                <Label htmlFor={field.name}>Management console access level</Label>
                <Select
                  name={field.name}
                  value={field.state.value}
