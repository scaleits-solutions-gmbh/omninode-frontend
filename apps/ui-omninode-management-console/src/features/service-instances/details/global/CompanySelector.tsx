...
export default function CompanySelector({ value, onValueChange }: CompanySelectorProps) {
          ...
          <div className="flex items-center gap-2">
            {value && (
              <Avatar className="h-6 w-6 rounded-sm">
                <AvatarImage 
                  src={companies.find((company) => company.value === value)?.imageUrl} 
                  alt={companies.find((company) => company.value === value)?.label} 
                />
                <AvatarFallback seed={value}>
                  {companies.find((company) => company.value === value)?.label.slice(0, 2)}
                </AvatarFallback>
              </Avatar>
            )}
            {value
              ...
              {companies.map((company) => (
                ...
                >
                  <Avatar className="h-6 w-6 rounded-sm">
                    ...
                    <AvatarFallback seed={company.value}>{company.label.slice(0, 2)}</AvatarFallback>
                  </Avatar>
                  {company.label}
                  <Check
                    className={cn(
