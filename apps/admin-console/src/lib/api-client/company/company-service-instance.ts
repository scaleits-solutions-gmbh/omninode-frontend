...
export function fetchCompanyServiceInstances({
  companyId,
  page,
  pageSize,
  search,
  service,
}: FetchCompanyServiceInstancesParams): Promise<
...
> {
  ...
  return apiClient.get<PaginatedResponse<FeCompanyServiceInstance>>(
    `/companies/${companyId}/service-instances`,
    queryParams,
  );
}
