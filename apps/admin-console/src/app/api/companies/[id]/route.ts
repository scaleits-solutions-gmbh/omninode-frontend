...
import { CompanyService, sanitizeWithZod, ResultType } from "@scaleits-solutions-gmbh/services";
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const sanitizedResponse = sanitizeWithZod<Company>(result, "Company");
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { result, resultType } = await companyService.updateCompanyByIdCurrent(id, body);
