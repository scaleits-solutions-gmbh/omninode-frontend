...
export async function POST(
  ...
  const { result, resultType } = await omninodeLoginService.switchCompany(userId, targetCompanyId);
  ...
  response.cookies.set("managementConsoleAccess", ManagementConsoleAccess.None, {
    httpOnly: false,
    secure: true,
    sameSite: "strict",
    domain: process.env.DOMAIN,
