...
import { CompanyRelationshipInviteService } from "@scaleits-solutions-gmbh/services";

export async function GET(request: NextRequest): Promise<NextResponse<feCompanyInvite|unknown>> {
    ...
    const { result, resultType } = await companyRelationshipInviteService.getCompanyRelationshipInvites({
        page: 1,
        pageSize: 1,
        token: token,
    });
